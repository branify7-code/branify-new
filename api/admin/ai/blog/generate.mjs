...
function maxTokensForLength(words) {
  return Math.min(16e3, Math.round(words * 2.6) + 1400);
}
function wordCountOf(html) {
  const plain = html.replace(/<[^>]*>/g, " ").replace(/[#>*`_\[\]()]/g, " ").replace(/\s+/g, " ").trim();
  return plain ? plain.split(" ").length : 0;
}
async function generateStructuredJson(cfg, systemPrompt, userPrompt, maxTokens) {
  const messages = [
  ...
  const repairMessages = [
    ...messages,
    { role: "assistant", content: raw.slice(0, 24e3) },
    {
      role: "user",
      content: "Your previous response was not parseable as a single JSON object. Return ONLY the corrected JSON object \u2014 no markdown fences, no commentary, no trailing text. Keep every field and the full article content from your previous answer."
    }
  ];
...
var LENGTH_HINTS = {
  ...
  2e3: "HARD MINIMUM 2000 words of article body (target 2000\u20132200). Do NOT stop early \u2014 a shorter submission is a failed response",
...
var BRANIFY_SYSTEM_PROMPT = `You are BRANIFY's senior SEO content strategist and editorial writer.
...
function buildUserPrompt(req) {
  ...
  lines.push(`Article length: ${LENGTH_HINTS[req.length] || `${req.length} words (\xB115%)`}`);
...
var GOALS = /* @__PURE__ */ new Set(["informational", "lead_generation", "service_promotion", "educational"]);
var TONES = /* @__PURE__ */ new Set(["professional", "expert", "conversational", "educational"]);
var LENGTHS = /* @__PURE__ */ new Set([1200, 1500, 2e3, 2500]);
function str(v, max) {
...
function validateRequest(body) {
  ...
  const existing = existingRaw.slice(0, 100).map((r) => {
...
function sanitizeArticleServerSide(html) {
  return html.replace(/<\s*(script|iframe|object|embed|style|svg|form)[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/gi, "").replace(/<\s*(script|iframe|object|embed|style|svg|form)[^>]*\/?>/gi, "").replace(/<img\b[^>]*>/gi, "").replace(/\son\w+\s*=\s*("[^"]*"|'[^']*'|[^\s>]+)/gi, "").replace(/\shref\s*=\s*("|')\s*(javascript|vbscript|data):[^"']*\1/gi, ' href="#"').replace(/\ssrc\s*=\s*("|')\s*(javascript|vbscript|data):[^"']*\1/gi, "");
}
function normalizeDraft(parsed, topic, keyword) {
  ...
  const seoObj = parsed.seo && typeof parsed.seo === "object" ? parsed.seo : {};
...
async function handler(req, res) {
  ...
  try {
    ...
    const err = e instanceof AiError ? e : new AiError("server_error", 500, e?.message?.slice(0, 300) || "Unexpected server error.");
