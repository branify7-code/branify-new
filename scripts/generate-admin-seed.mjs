...
// --------------------------------------------------------- map: templates
// Generated TemplateRecord entries (Flow AI mockups, drop 1+2+3 minus removals).
...
-- PREREQUISITE: run supabase/admin-schema.sql FIRST — this file only seeds rows
--   and creates NO tables. If you hit ERROR: relation "public.templates" does
--   not exist (or any other missing relation), apply admin-schema.sql, then
--   re-run this file. Idempotent: existing slugs are left untouched.
-- =============================================================================\n`);

// Owner-requested removals (2026-09): the public registry no longer ships these
// rows, so a seed re-run must remove them from prod too (emitted BEFORE the
// inserts so removed rows can never be resurrected by this file).
sqlParts.push(`-- Owner-requested removals (2026-09) — keep prod in sync with the live registry
delete from public.templates where slug = 'kado-steakhouse';
delete from public.template_categories where slug = 'cafe-coffee';
\n`);
