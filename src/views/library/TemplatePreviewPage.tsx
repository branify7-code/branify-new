...
const TemplatePreviewPage: React.FC<TemplatePreviewPageProps> = ({ categorySlug, templateSlug, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* Toolbar */}
      <div className="sticky top-0 z-40 bg-white/85 backdrop-blur-xl border-b border-[#E2E8F0]">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest bg-white border border-[#E2E8F0] shadow-[0_2px_10px_rgba(15,23,42,0.04)] text-[#475569] hover:border-[#C9A45C]/45 hover:text-[#8F6B2D] transition-all cursor-pointer shrink-0"
            >
            ...
            <div className="min-w-0 hidden md:block">
              <p className="text-xs font-black text-[#111827] uppercase tracking-tight truncate">{template.name}</p>
              <p className="text-[10px] font-mono text-[#64748B] truncate">BRANIFY Template Library{category ? ` · ${category.name}` : ''}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest bg-white border border-[#E2E8F0] shadow-[0_2px_10px_rgba(15,23,42,0.04)] text-[#475569] hover:border-[#C9A45C]/45 hover:text-[#8F6B2D] transition-all cursor-pointer"
            ...
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest bg-white border border-[#E2E8F0] shadow-[0_2px_10px_rgba(15,23,42,0.04)] text-[#475569] hover:border-[#C9A45C]/45 hover:text-[#8F6B2D] transition-all"
            >
      ...
      <div className="flex-1 overflow-auto">
        <div className={`mx-auto py-6 px-4 ${actualSize ? 'w-max' : 'max-w-[1400px]'}`}>
          <img
            ...
            height={900}
            decoding="async"
            className={`rounded-2xl border border-[#E2E8F0] shadow-[0_30px_80px_-24px_rgba(15,23,42,0.28)] ${actualSize ? 'w-[1200px] max-w-none' : 'w-full h-auto'}`}
          />
          <div className="h-6" />
          <p className="text-center text-[10px] font-bold uppercase tracking-[0.3em] text-[#64748B] pb-6">
